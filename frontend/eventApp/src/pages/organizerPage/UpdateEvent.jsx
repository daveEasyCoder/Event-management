...
const EditVenue = () => {
  ...
  useEffect(() => {
    const fetchVenue = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${BASE_URL}/api/venue/${id}`, { withCredentials: true });
        ...
        if (response.data.success) {
          ...
          // Set current image
          if (venue.image) {
            setCurrentImage(`${BASE_URL}/uploads/venues/${venue.image}`);
          }
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Image is optional for update (can keep existing)
    try {
      setUpdating(true);
