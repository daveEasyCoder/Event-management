    ...
    const handleSubmit = async (e) => {
        ...
        if(!email){
            ...
            try {
                ...
                const res = await axios.post(`${BASE_URL}/api/users/login`, {email,password}, {withCredentials:true});
                if(res.data.success){
                ...
                if(err?.response){
                   if(err.response.data.message){
                    ...
                    setMessage("Server is not responding. try later!")
                }
            }
        }
    }
    ...
    <div className="w-full flex flex-col items-center justify-center">
        ...
        <form onSubmit={handleSubmit} className="md:w-96 w-80 flex flex-col items-center justify-center">
            ...
            <div className="flex items-center w-full bg-transparent border border-gray-300/60 h-12 rounded-full overflow-hidden pl-6 gap-2">
                ...
                <input type="email" onChange={(e) => setEmail(e.target.value)} placeholder="Email id" className="bg-transparent text-gray-500/80 placeholder-gray-500/80 outline-none text-sm w-full h-full" required />                 
            </div>
            ...
            <p className="text-gray-500/90 text-sm mt-4">Don’t have an account? <Link to = "/register" className="text-indigo-400 hover:underline" href="#">Sign up</Link></p>
        </form>
    </div>
   </div>
  )
